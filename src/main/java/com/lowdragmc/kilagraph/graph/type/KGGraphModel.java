...
public class KGGraphModel extends CustomGraphModelImpl {
     ...
     *
     * <p>{@code beforeSerialize()} rather than 26.1-has-no-such-thing {@code serializeNBT}: the
     * model layer serializes through {@code PersistedParser}, which calls this on the root object
     * first and {@code serializeAdditionalNBT} (where {@code GraphModel} writes the nodes) after —
     * so this is the last moment at which an option constant can still be changed and be saved.</p>
     */
    @Override
