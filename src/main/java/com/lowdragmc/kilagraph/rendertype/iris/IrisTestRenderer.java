 ...
 * dirt-textured lit entity shader), submitted through {@link SubmitCustomGeometryEvent} — the very same
 * {@code SubmitNodeCollector} → {@code RenderType.prepare()} → {@code drawFromBuffer} path a block-entity
 * renderer takes. So it exercises the exact Iris routing ({@code assignPipeline} + the injected gbuffers
 * program). If this cube shows under a shaderpack but a host's hologram doesn't, the difference is the
 * host's draw path; if neither shows, the bug is reproducible here in code we control.
 */
...
public final class IrisTestRenderer {
    ...
    /** When true, draw with a vanilla {@code RenderTypes.entitySolid} instead of our KilaGraph material —
    ...
    private static void submitCube(SubmitCustomGeometryEvent event, RenderType renderType) {
        ...
        event.getSubmitNodeCollector().submitCustomGeometry(poseStack, renderType, (pose, vc) -> {
