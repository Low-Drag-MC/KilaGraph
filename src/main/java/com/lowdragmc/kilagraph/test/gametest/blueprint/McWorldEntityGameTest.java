...
public final class McWorldEntityGameTest {
    private static final String WORLD_QUERIES = "mc_world_entity_world_queries";
    private static final String RAYCAST_FINDS_A_PLACED_BLOCK = "mc_world_entity_raycast_finds_a_placed_block";
    private static final String ENTITY_QUERIES_AND_DATA = "mc_world_entity_entity_queries_and_data";
    private static final String ENCHANTMENT_PROBE_USES_THE_WORLD_REGISTRY = "mc_world_entity_enchantment_probe_uses_the_world_registry";

    public static void registerFunctions() {
        KGGameTests.registerFunction(WORLD_QUERIES, McWorldEntityGameTest::worldQueries);
        KGGameTests.registerFunction(RAYCAST_FINDS_A_PLACED_BLOCK, McWorldEntityGameTest::raycastFindsAPlacedBlock);
        KGGameTests.registerFunction(ENTITY_QUERIES_AND_DATA, McWorldEntityGameTest::entityQueriesAndData);
        KGGameTests.registerFunction(ENCHANTMENT_PROBE_USES_THE_WORLD_REGISTRY, McWorldEntityGameTest::enchantmentProbeUsesTheWorldRegistry);
    }
    ...
    public static void worldQueries(GameTestHelper helper) {
        ...
        assertEq(helper, "the biome has an id", true,
                probe(level, WorldQueryNodes.BiomeId.class, "pos", abs)
                        .eval("out", Identifier.class) != null);
        ...
        assertTrue(helper, "an open column sees the sky",
                probe(level, WorldQueryNodes.CanSeeSky.class, "pos", helper.absolutePos(new BlockPos(0, 5, 0)))
                        .eval("out", Boolean.class));

        assertEq(helper, "dimension id", level.dimension().identifier(),
                probe(level, WorldQueryNodes.DimensionId.class).eval("out", Identifier.class));
    ...
    public static void raycastFindsAPlacedBlock(GameTestHelper helper) {
        ...
        var miss = probe(level, WorldQueryNodes.RaycastBlock.class,
                "from", new Vector3f(target.getX() + 0.5f, target.getY() + 40f, target.getZ() + 0.5f),
                "to", new Vector3f(target.getX() + 0.5f, target.getY() + 30f, target.getZ() + 0.5f));
    ...
    public static void entityQueriesAndData(GameTestHelper helper) {
        ...
        var speed = probe(level, EntityDataNodes.Attribute.class, "entity", pig,
                "attribute", Identifier.parse("minecraft:movement_speed"));
        ...
        var effect = probe(level, EntityDataNodes.HasEffect.class, "entity", pig,
                "effect", Identifier.parse("minecraft:speed"));
    ...
    public static void enchantmentProbeUsesTheWorldRegistry(GameTestHelper helper) {
        ...
        var sharp = probe(level, RegistryProbeNodes.EnchantmentExists.class,
                "id", Identifier.parse("minecraft:sharpness"));
        ...
        var nope = probe(level, RegistryProbeNodes.EnchantmentExists.class,
                "id", Identifier.parse("kilagraph:no_such_enchantment"));
