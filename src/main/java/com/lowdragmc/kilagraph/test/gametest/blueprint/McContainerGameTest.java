 ...
 * the capability the nodes used. If both sides went through {@code ResourceHandler}, a bug in how this
 * mod reads the inventory would agree with itself and the test would pass.
 */
public final class McContainerGameTest {
    private static final String RESOLVES_A_CHEST_AND_READS_IT = "mc_container_resolves_a_chest_and_reads_it";
    private static final String A_STONE_BLOCK_IS_NOT_A_CONTAINER = "mc_container_a_stone_block_is_not_a_container";
    private static final String INSERTS_AND_EXTRACTS = "mc_container_inserts_and_extracts";
    private static final String A_FULL_CHEST_REPORTS_THE_REMAINDER = "mc_container_a_full_chest_reports_the_remainder";
    private static final String A_PLAYER_INVENTORY_IS_A_CONTAINER = "mc_container_a_player_inventory_is_a_container";
    private static final String BLOCK_ENTITY_NBT_ROUND_TRIPS = "mc_container_block_entity_nbt_round_trips";
    private static final String READS_AND_DRAINS_A_CAULDRON = "mc_container_reads_and_drains_a_cauldron";
    private static final String FLUID_TANKS_DEGRADE_ON_BLOCKS_THAT_HAVE_NONE = "mc_container_fluid_tanks_degrade_on_blocks_that_have_none";
    private static final String ENTITY_NBT_ROUND_TRIPS_WITHOUT_TELEPORTING = "mc_container_entity_nbt_round_trips_without_teleporting";

    public static void registerFunctions() {
        KGGameTests.registerFunction(RESOLVES_A_CHEST_AND_READS_IT, McContainerGameTest::resolvesAChestAndReadsIt);
        KGGameTests.registerFunction(A_STONE_BLOCK_IS_NOT_A_CONTAINER, McContainerGameTest::aStoneBlockIsNotAContainer);
        KGGameTests.registerFunction(INSERTS_AND_EXTRACTS, McContainerGameTest::insertsAndExtracts);
        KGGameTests.registerFunction(A_FULL_CHEST_REPORTS_THE_REMAINDER, McContainerGameTest::aFullChestReportsTheRemainder);
        KGGameTests.registerFunction(A_PLAYER_INVENTORY_IS_A_CONTAINER, McContainerGameTest::aPlayerInventoryIsAContainer);
        KGGameTests.registerFunction(BLOCK_ENTITY_NBT_ROUND_TRIPS, McContainerGameTest::blockEntityNbtRoundTrips);
        KGGameTests.registerFunction(READS_AND_DRAINS_A_CAULDRON, McContainerGameTest::readsAndDrainsACauldron);
        KGGameTests.registerFunction(FLUID_TANKS_DEGRADE_ON_BLOCKS_THAT_HAVE_NONE, McContainerGameTest::fluidTanksDegradeOnBlocksThatHaveNone);
        KGGameTests.registerFunction(ENTITY_NBT_ROUND_TRIPS_WITHOUT_TELEPORTING, McContainerGameTest::entityNbtRoundTripsWithoutTeleporting);
    }
    ...
    public static void register(RegisterGameTestsEvent event, Holder<TestEnvironmentDefinition<?>> environment) {
        ...
        for (String p : new String[]{
                ...
                READS_AND_DRAINS_A_CAULDRON, FLUID_TANKS_DEGRADE_ON_BLOCKS_THAT_HAVE_NONE, ENTITY_NBT_ROUND_TRIPS_WITHOUT_TELEPORTING
        }) {
    ...
    public static void insertsAndExtracts(GameTestHelper helper) {
        ServerLevel level = helper.getLevel();
        BlockPos chest = helper.absolutePos(new BlockPos(1, 2, 1));
        level.setBlock(chest, Blocks.CHEST.defaultBlockState(), 3);
        ...
        // No set-slot coverage: 26.1's transfer API has no overwrite operation, so mc_container_set
        // does not exist here. Emptying a slot and inserting is the replacement, and both halves of
        // that are already covered above.
     ...
     *
     * <p>A cauldron turns out to be a real fluid handler: NeoForge registers the capability for one, so
     * vanilla does have a tank to test against after all. The assertions are relational rather than
     * absolute — that draining yields water, that the cauldron empties, that filling it back fills it —
     * because a cauldron's exact capacity and level granularity are NeoForge's business and asserting
     * them would be testing NeoForge.
     *
    ...
    public static void readsAndDrainsACauldron(GameTestHelper helper) {
        ...
        // Whether a cauldron's wrapper reports anything drainable under SIMULATE is NeoForge's business
        ...
        // covered on a chest in insertsAndExtracts, where the handler is one that implements it.
