...
/**
 * Rotations: that the quaternion nodes agree with the vector ones about which way is which, and that
 * the two representations round-trip.
 *
 * <p><b>Orientations are compared as orientations</b>, not as four numbers — {@code q} and
 * {@code -q} are the same rotation, so {@link #assertSame} compares |dot| rather than components.
 * A test written the other way passes or fails depending on which branch of a square root an
 * implementation happened to take.</p>
 *
 * <p>The cross-checks against {@code vector_from_rotation} and {@code vector_rotate_axis} are the
 * point of the file: a sign convention that is wrong in both places is invisible inside either.</p>
 */
public final class QuaternionNodeGameTest {
    private static final String FROM_EULER = "quat_from_euler_matches_vector_from_rotation";
    private static final String EULER_ROUND_TRIP = "quat_euler_round_trips";
    private static final String AXIS_ANGLE = "quat_axis_angle_matches_rotate_about_axis";
    private static final String MULTIPLY = "quat_multiply_composes_right_to_left";
    private static final String IDENTITY = "quat_identity_is_no_rotation";
    private static final String INVERSE = "quat_inverse_undoes_the_rotation";
    private static final String SLERP = "quat_slerp_takes_the_short_arc";
    private static final String FROM_TO = "quat_from_to_turns_one_direction_into_another";
    private static final String ANGLE_BETWEEN = "quat_angle_between_is_the_shortest_turn";
    private static final String VEC4_BRIDGE = "quat_vec4_bridge_round_trips";
    private static final String PIN_TYPES = "quat_pins_do_not_accept_vectors";
    private static final String DEGENERATE = "quat_degenerate_inputs_answer";

    public static void registerFunctions() {
        KGGameTests.registerFunction(FROM_EULER, QuaternionNodeGameTest::fromEulerMatchesVectorFromRotation);
        KGGameTests.registerFunction(EULER_ROUND_TRIP, QuaternionNodeGameTest::eulerRoundTrips);
        KGGameTests.registerFunction(AXIS_ANGLE, QuaternionNodeGameTest::fromAxisAngleMatchesRotateAboutAxis);
        KGGameTests.registerFunction(MULTIPLY, QuaternionNodeGameTest::multiplyComposesRightToLeft);
        KGGameTests.registerFunction(IDENTITY, QuaternionNodeGameTest::identityIsNoRotation);
        KGGameTests.registerFunction(INVERSE, QuaternionNodeGameTest::inverseUndoesTheRotation);
        KGGameTests.registerFunction(SLERP, QuaternionNodeGameTest::slerpTakesTheShortArc);
        KGGameTests.registerFunction(FROM_TO, QuaternionNodeGameTest::fromToTurnsOneDirectionIntoTheOther);
        KGGameTests.registerFunction(ANGLE_BETWEEN, QuaternionNodeGameTest::angleBetweenIsTheShortestTurn);
        KGGameTests.registerFunction(VEC4_BRIDGE, QuaternionNodeGameTest::theVec4BridgeRoundTripsExactly);
        KGGameTests.registerFunction(PIN_TYPES, QuaternionNodeGameTest::quatPinsDoNotAcceptVectorsOrViceVersa);
        KGGameTests.registerFunction(DEGENERATE, QuaternionNodeGameTest::degenerateInputsAnswerRatherThanFail);
    }

    public static void register(RegisterGameTestsEvent event, Holder<TestEnvironmentDefinition<?>> environment) {
        var data = KGGameTests.defaultTestData(environment);
        for (String p : new String[]{
                FROM_EULER, EULER_ROUND_TRIP, AXIS_ANGLE, MULTIPLY, IDENTITY, INVERSE, SLERP,
                FROM_TO, ANGLE_BETWEEN, VEC4_BRIDGE, PIN_TYPES, DEGENERATE
        }) {
