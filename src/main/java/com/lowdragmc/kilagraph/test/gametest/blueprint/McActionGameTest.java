...
public final class McActionGameTest {
    private static final String SET_AND_BREAK_BLOCK = "mc_action_set_and_break_block";
    private static final String FILL_AND_REPLACE_BLOCKS = "mc_action_fill_and_replace_blocks";
    private static final String SPAWN_MOVE_AND_REMOVE_ENTITY = "mc_action_spawn_move_and_remove_entity";
    private static final String DAMAGE_HEAL_AND_EFFECT = "mc_action_damage_heal_and_effect";
    private static final String SOUND_AND_PARTICLES = "mc_action_sound_and_particles";
    private static final String DROP_ITEM = "mc_action_drop_item";
    private static final String RUNS_A_COMMAND = "mc_action_runs_a_command";

    public static void registerFunctions() {
        KGGameTests.registerFunction(SET_AND_BREAK_BLOCK, McActionGameTest::setAndBreakBlock);
        KGGameTests.registerFunction(FILL_AND_REPLACE_BLOCKS, McActionGameTest::fillAndReplaceBlocks);
        KGGameTests.registerFunction(SPAWN_MOVE_AND_REMOVE_ENTITY, McActionGameTest::spawnMoveAndRemoveEntity);
        KGGameTests.registerFunction(DAMAGE_HEAL_AND_EFFECT, McActionGameTest::damageHealAndEffect);
        KGGameTests.registerFunction(SOUND_AND_PARTICLES, McActionGameTest::soundAndParticles);
        KGGameTests.registerFunction(DROP_ITEM, McActionGameTest::dropItem);
        KGGameTests.registerFunction(RUNS_A_COMMAND, McActionGameTest::runsACommand);
    }
    ...
    public static void damageHealAndEffect(GameTestHelper helper) {
        ...
        var effect = run(level, EntityActionNodes.AddEffect.class, "entity", pig,
                "effect", Identifier.parse("minecraft:speed"), "duration", 200, "amplifier", 1);
        assertTrue(helper, "effect reported success", effect.ok());
        var speed = pig.getEffect(MobEffects.SPEED);
    ...
    public static void soundAndParticles(GameTestHelper helper) {
        ...
        assertTrue(helper, "a known sound plays",
                run(level, WorldEffectNodes.PlaySound.class, "pos", at,
                        "sound", Identifier.parse("minecraft:block.anvil.land")).ok());
        assertFalse(helper, "an unknown sound id is refused",
                run(level, WorldEffectNodes.PlaySound.class, "pos", at,
                        "sound", Identifier.parse("kilagraph:no_such_sound")).ok());

        assertTrue(helper, "a simple particle spawns",
                run(level, WorldEffectNodes.SpawnParticle.class, "pos", at,
                        "particle", Identifier.parse("minecraft:flame"), "count", 4).ok());
        ...
        assertFalse(helper, "a parameterised particle is refused",
                run(level, WorldEffectNodes.SpawnParticle.class, "pos", at,
                        "particle", Identifier.parse("minecraft:dust"), "count", 1).ok());
        assertFalse(helper, "an unknown particle id is refused",
                run(level, WorldEffectNodes.SpawnParticle.class, "pos", at,
                        "particle", Identifier.parse("kilagraph:no_such_particle")).ok());
        helper.succeed();
    }
    ...
    public static void runsACommand(GameTestHelper helper) {
        ...
        // "time query gametime" is chosen because both halves are checkable: its feedback text is
        ...
        var query = run(level, RunCommandNode.class, "command", "time query gametime");
        assertTrue(helper, "time query succeeded", query.ok());
        assertEq(helper, "and the result is the game time",
                (int) (level.getGameTime() % 2147483647L), query.get("result", Integer.class).intValue());
        assertTrue(helper, "with feedback text captured, got: " + query.get("output", String.class),
                query.get("output", String.class).contains(String.valueOf(level.getGameTime())));
