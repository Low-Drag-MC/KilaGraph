...
public final class UIElementNodes {
    ...
    public static class FromXml extends AnnotatedNode {
        ...
        public void execute(ExecContext ctx) {
            ...
            UIActions.produce(ctx, "element", parsed.isEmpty() ? null : parsed.getFirst());
    ...
    public static class Serialize extends AnnotatedNode {
        ...
        public void evaluate(EvalContext ctx) {
            UIElement element = UIActions.element(ctx, "element");
            if (element == null) {
                ctx.setOutput("nbt", new CompoundTag());
                return;
            }
            // 26.1 writes through ValueOutput; TagValueOutput is the CompoundTag-backed one, and this is
            // the pair UITemplate.of uses to snapshot a tree.
            var output = TagValueOutput.createWithContext(
                    ProblemReporter.DISCARDING, Platform.getFrozenRegistry());
            element.serialize(output);
            ctx.setOutput("nbt", output.buildResult());
        }
    }
    ...
    public static class Deserialize extends AnnotatedNode {
        ...
        public void execute(ExecContext ctx) {
            ...
            element.deserialize(TagValueInput.create(
                    ProblemReporter.DISCARDING, Platform.getFrozenRegistry(), nbt));
            UIActions.done(ctx, true);
        }
    }
