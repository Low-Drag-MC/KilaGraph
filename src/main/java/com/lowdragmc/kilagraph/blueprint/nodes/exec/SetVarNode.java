...
public class SetVarNode extends AnnotatedNode {
     ...
     * Called from {@code KGGraphModel.beforeSerialize} — the same moment, and for the same reason,
     * that a custom event's callers write down the signature they mirror.
     */
    public static void syncDeclarations(GraphModel graphModel) {
        for (var model : graphModel.getNodeModels()) {
            if (!(model instanceof NodeModel node)
                    || !(node instanceof ICustomNodeModel custom)
