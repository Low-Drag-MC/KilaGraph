...
public final class LevelInfoBlocks {
    ...
    public static class Time extends InfoPropertyBlock<Level> {
        ...
        @Override
        protected void read(Level level, EvalContext ctx) {
            ctx.setOutput("dayTime", level.getOverworldClockTime());
            ctx.setOutput("gameTime", level.getGameTime());
            ctx.setOutput("day", level.isBrightOutside());
            ctx.setOutput("night", level.isDarkOutside());
        }
    }
    ...
    public static class Dimension extends InfoPropertyBlock<Level> {
        @OutputPort public Identifier value;
        ...
        @Override
        protected void read(Level level, EvalContext ctx) {
            ctx.setOutput("value", level.dimension().identifier());
        }
    }
    ...
    public static class Bounds extends InfoPropertyBlock<Level> {
        @OutputPort public int minY;
        @OutputPort public int maxY;
        ...
        @Override
        protected void read(Level level, EvalContext ctx) {
            // 26.1 names these getMinY/getMaxY and makes maxY INCLUSIVE, where 1.21.1's
            // getMaxBuildHeight() was one past the top. The ports follow 26.1 rather than
            // reproducing the old off-by-one under the old name.
            ctx.setOutput("minY", level.getMinY());
            ctx.setOutput("maxY", level.getMaxY());
