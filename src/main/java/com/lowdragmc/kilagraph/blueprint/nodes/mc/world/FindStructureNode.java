...
public class FindStructureNode extends AnnotatedNode {
    ...
    @InputPort public Identifier structure;
    ...
    private static BlockPos search(EvalContext ctx, BlockPos from) {
        ...
        // 26.1 moved the world-gen options off WorldData onto the server's WorldGenSettings.
        if (!world.getServer().getWorldGenSettings().options().generateStructures()) return null;
        Identifier id = ctx.getInput("structure", Identifier.class, null);
        ...
        int radius = Math.clamp(ctx.getInt("radius", 100), 1, MAX_RADIUS);
