...
/**
 * The four periodic waveforms, in one dropdown. All share the same two properties: <b>period 1</b> in
 * {@code in} and <b>range [-1, 1]</b> — so a graph can change the shape of an animation without
 * rescaling the driver or the consumer.
 *
 * <p>The phase is each waveform's own, and is not aligned: at {@code in == 0} sine and sawtooth are
 * 0 (rising), square is +1 and triangle is -1. Offset {@code in} if a particular start matters.</p>
 */
