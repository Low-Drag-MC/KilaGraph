...
public final class EntityInteractionNodes {
    ...
    public static class Mount extends AnnotatedNode {
        ...
        public void execute(ExecContext ctx) {
            ...
            // 26.1 split the old 2-arg form: (force, sendEventAndTriggers). The 1.21 call passed
            // force and got the events, so both are true here.
            McActions.done(ctx, rider.startRiding(vehicle, true, true));
        }
    }
    ...
    public static class SetNbt extends AnnotatedNode {
        ...
        public void execute(ExecContext ctx) {
            ...
            // 26.1 reads entity NBT through ValueInput; DISCARDING drops parse problems this node
            // has nowhere to report.
            e.load(TagValueInput.create(ProblemReporter.DISCARDING, e.level().registryAccess(), tag));
