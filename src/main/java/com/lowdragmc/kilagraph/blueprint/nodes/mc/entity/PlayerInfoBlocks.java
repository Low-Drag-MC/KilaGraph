...
public final class PlayerInfoBlocks {
     ...
     * the bar does, which is why a player who just ate looks unchanged for a while.</p>
     *
     * <p>There is no {@code exhaustion} output: 26.1 removed {@code FoodData.getExhaustionLevel()} and
     * left the field private, so the value is no longer readable through any supported API. The 1.21
     * branch exposes it; here it is gone rather than faked.</p>
     */
