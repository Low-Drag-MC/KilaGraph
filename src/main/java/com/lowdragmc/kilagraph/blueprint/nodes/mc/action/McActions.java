...
public final class McActions {
    ...
    /**
     * Run one item/fluid transfer and honour the node's {@code simulate} input.
     *
     * <p>26.1 replaced the old per-call {@code simulate} boolean with transactions, and the translation
     * is the same every time: open a root transaction, do the work, and commit it only if the graph did
     * not ask for a dry run. A transaction that is closed without committing rolls back, so "simulate"
     * is simply "never commit" — which is one rule the node set makes once here rather than four times
     * across two packages.</p>
     *
     * @param transfer the work to do, given the open transaction; its result is returned unchanged
     */
    public static <T> T transfer(ExecContext ctx, Function<TransactionContext, T> transfer) {
        try (Transaction tx = Transaction.openRoot()) {
            T result = transfer.apply(tx);
            if (!ctx.getBool("simulate", false)) tx.commit();
            return result;
        }
    }
}
